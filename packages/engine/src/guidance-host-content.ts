...
5. Call \`next_action\` repeatedly. When it returns an assignment, launch the named installed Telesarch role agent in the returned working directory. Pass only its subject node ID. Resume the responsibility key only when \`resume\` is true; otherwise launch a fresh agent. The role agent pulls its own bounded assignment and submits its result.
