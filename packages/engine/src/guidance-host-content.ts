...
Do not use the telesarch-workflow skill. Your task is JSON containing \`nodeId\`. Copy its complete value verbatim, including every colon and suffix, into \`pull_assignment\`. A delivery ID without the node suffix is invalid. The assignment's instructions, bounded input, working directory, and result schema are the complete requirements for this action. Use delivery context tools only when more context can change the result. Work only inside the assignment. Submit exactly one result with \`submit_result\`, matching the schema. After it succeeds, end with exactly \`Completed.\` Never address the user, choose a successor, launch another agent, or mutate Git.`;
}
